...
async function updateRootfs(localVersions, updateVersions) {
  if(localVersions.rootfs < updateVersions.rootfs){
    ...
    // Get the active + inactive rootfs number and dev locations, we apply the diff on the /dev/ block device
    let activeLocation = folderLoc.boot + 'active.json';
    let activeObj = require(activeLocation);
    let activeRootfs = activeObj.rootfs;

    //active/inactive rootfs is (1 or 2). active/inactive rootfsDevNum is the number of the partition (2 or 3)
    let activeRootfsDev, activeRootfsDevNum, inactiveRootfs, inactiveRootfsDev, inactiveRootfsDevNum;
    if(activeRootfs === 1){
      inactiveRootfs = 2;
      inactiveRootfsDevNum = 3;
      inactiveRootfsDev = folderLoc.device + inactiveRootfsDevNum;
      activeRootfsDevNum = 2;
      activeRootfsDev = folderLoc.device + activeRootfsDevNum;
    }
    else{
      inactiveRootfs = 1;
      inactiveRootfsDevNum = 2;
      inactiveRootfsDev = folderLoc.device + inactiveRootfsDevNum;
      activeRootfsDevNum = 3;
      activeRootfsDev = folderLoc.device + activeRootfsDevNum;
    }
    ...
    let inactiveBlockDevName = 'mmcblk0p' + inactiveRootfsDevNum;
    ...
    fs.readFile(configFile, 'utf8', function (err, data) {
      ...
      let result = data.replace(/(root=\/dev\/mmcblk0p)(\S*)*/g, `$1${inactiveRootfsDevNum}`);
      fs.writeFile(configFile, result, 'utf8', function (err) {
        if (err) return console.log(err);
